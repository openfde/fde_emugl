...
#include <syslog.h>
#include <sys/time.h>
//#define CHECK_GL_OPCODE
//#define CHECK_GL_OPCODE_TIME_USE
...
int renderControl_decoder_context_t::decode(void *buf, size_t len, IOStream *stream, ChecksumCalculator* checksumCalc) {
    if (len < 8) return 0; 
#ifdef CHECK_GL_ERRORS
    char lastCall[256] = {0};
#endif
    ...
    while (end - ptr >= 8) {
...
#ifdef CHECK_GL_OPCODE_TIME_USE
        ...
        case OP_rcTransferAppStatus: {        
            int32_t display_id = Unpack<int32_t,int32_t>(ptr + 8);
            int32_t transfer_flags = Unpack<int32_t,int32_t>(ptr + 8 + 4);
            FrameBuffer *fb = FrameBuffer::getFB();                            
            fb->setDisplayShowStatus(display_id, transfer_flags);
            
            char * result = "transferOK";
            unsigned char *tmpBuf = stream->alloc(strlen(result)+1);            
            memcpy((char *)tmpBuf, result, strlen(result)+1);
            stream->flush();            
            SET_LASTCALL("OP_rcTransferAppStatus");
            break;            
        }        
        case OP_rcCheckRenderThread: {            
            int32_t check_flags = Unpack<int32_t,int32_t>(ptr + 8);            
            char * result = "OK";
            unsigned char *tmpBuf = stream->alloc(strlen(result)+1);            
            memcpy((char *)tmpBuf, result, strlen(result)+1);
            stream->flush();            
            SET_LASTCALL("OP_rcCheckRenderThread");
            break;            
        }        
        case OP_rcPostVirtualDisplay: {
            //uint32_t var_colorBuffer = Unpack<uint32_t,uint32_t>(ptr + 8);
            //8 + __size_name + 4 + 4 + 4 + 4 + 4 + 4 + 4 + 1*4

            uint32_t size_name __attribute__((unused)) = Unpack<uint32_t,uint32_t>(ptr + 8);
            InputBuffer inptr_name(ptr + 8 + 4 , size_name);
            //uint32_t _size_name = packetLen - (8 + 4 + 4 + 4 + 4 + 4 + 4 + 4 + 1*4)
            uint32_t var_display_id = Unpack<uint32_t,uint32_t>(ptr + 8 + 4 + size_name);
            uint32_t var_colorBuffer = Unpack<uint32_t,uint32_t>(ptr + 8 + 4 + size_name + 4);
            int32_t var_width = Unpack<int32_t,int32_t>(ptr + 8 + 4 + size_name + 4 + 4);
            int32_t var_height = Unpack<int32_t,int32_t>(ptr + 8 + 4 + size_name + 4 + 4 + 4);
            int32_t var_stride = Unpack<int32_t,int32_t>(ptr + 8 + 4 + size_name + 4 + 4 + 4 + 4);
            int32_t var_bpp = Unpack<int32_t,int32_t>(ptr + 8 + 4 + size_name + 4 + 4 + 4 + 4 + 4);
            int32_t var_orientation = Unpack<int32_t,int32_t>(ptr + 8 + 4 + size_name + 4 + 4 + 4 + 4 + 4 + 4);

            //syslog(LOG_DEBUG,"postVirtualDisplay... %s",(char*)(inptr_name.get()));
            //syslog(LOG_DEBUG,"postVirtualDisplay var_display_id = %d,var_colorBuffer = %d,var_width = %d,var_height = %d,var_stride = %d,var_bpp = %d,var_orientation = %d",var_display_id,var_colorBuffer,var_width,var_height,var_stride,var_bpp,var_orientation);


            //int32_t width, int32_t height, int32_t stride, int32_t bpp, int32_t orientation
            //const char* name, uint32_t id, uint32_t bo_name, int32_t width, int32_t height, int32_t stride, int32_t bpp, int32_t orientation
            if (useChecksum) {
                ChecksumCalculatorThreadInfo::validOrDie(checksumCalc, ptr, 8 + 4, ptr + 8 + 4, checksumSize, 
                    "renderControl_decoder_context_t::decode, OP_postVirtualDisplay: GL checksumCalculator failure\n");
            }
            //DEBUG("renderControl(%p): rcFBPost(0x%08x )\n", stream, var_colorBuffer);
            this->rcPostVirtualDisplay((char*)inptr_name.get(),var_display_id,var_colorBuffer,var_width,var_height,var_stride,var_bpp,var_orientation);
            SET_LASTCALL("rcPostVirtualDisplay");
            break;            
        }
        case OP_rcPostAllVirtualDisplaysDone: {
            //syslog(LOG_DEBUG,"postAllVirtualDisplaysDone...");
            
            size_t totalTmpSize = sizeof(GLint);
            totalTmpSize += checksumSize;
            unsigned char *tmpBuf = stream->alloc(totalTmpSize);
            //DEBUG("renderControl(%p): rcSetColorBufferVulkanMode(0x%08x 0x%08x )\n", stream, var_colorBuffer, var_mode);
            this->rcPostAllVirtualDisplaysDone();
            *(GLint *)(&tmpBuf[0]) = 0;
            if (useChecksum) {
                ChecksumCalculatorThreadInfo::writeChecksum(checksumCalc, &tmpBuf[0], totalTmpSize - checksumSize, &tmpBuf[totalTmpSize - checksumSize], checksumSize);
            }
            stream->flush();
            SET_LASTCALL("rcPostAllVirtualDisplaysDone");
            break;
        }
        case OP_rcPostCaptureScreenColorBuffer: {
            int32_t cb_handle = Unpack<int32_t,int32_t>(ptr + 8);
            this->rcPostCaptureScreenColorBuffer(cb_handle);
            SET_LASTCALL("rcPostCaptureScreenColorBuffer");
            break;
        }
        default:
...
#ifdef CHECK_GL_OPCODE
        if (opcode > 10000 && opcode < 20000) {
            syslog(LOG_DEBUG,"GGGG,gles1 opcode = %d",opcode);
        }
#endif
        ...
        if (opcode > 10000 && opcode < 20000) {
            ...
            if (time_use > 0) {                
                syslog(LOG_DEBUG,"GGGG, time_use = %ld",time_use);
            }
        }
#endif
