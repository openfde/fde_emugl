...
static thread_local std::shared_ptr<GrabberHelper> pGrabberHelper;
...
ColorBuffer::ColorBuffer(EGLDisplay display, HandleType hndl, Helper* helper)
    ...
    m_pre_record_time(0), m_record_counter(0){}

ColorBuffer::~ColorBuffer() {
    FrameBuffer *fb = FrameBuffer::getFB();
    if (fb) {
        fb->removeCaptureScreenColorBuffer(mHndl);
    }
...
ColorBuffer::RecordStatus ColorBuffer::getRecordStatus()
{
    m_record_counter++;
    //syslog(LOG_DEBUG, "[ColorBuffer][%s] record_counter = %d\n", __func__, m_record_counter);
    if (m_record_counter <= 3) { // skip first 3 frames
        m_pre_record_time = get_micro_time();
        return Record_Stop;
    }
    else if (m_record_counter == 4) {
        uint64_t now_record_time = get_micro_time();
        if ((now_record_time - m_pre_record_time) > 3000000) { // invalid counting, reset counter
            m_record_counter = 0;
            m_pre_record_time = get_micro_time();
            //syslog(LOG_DEBUG, "[ColorBuffer][%s] invalid counting, reset counter\n", __func__);
            return Record_Stop;
        }
        m_pre_record_time = now_record_time;
        //syslog(LOG_DEBUG, "[ColorBuffer][%s] record start...\n", __func__);
        return Record_Start;
    }
    else {
        uint64_t now_record_time = get_micro_time();
        if ((now_record_time - m_pre_record_time) > 500000) {// recording has stoped before, so reset counter
            m_record_counter = 0;
            //syslog(LOG_DEBUG, "[ColorBuffer][%s] record stoped, reset counter!\n", __func__);
            return Record_Stop;
        }
        m_pre_record_time = now_record_time;
        return Record_Ongoing;
    }
}
...
void ColorBuffer::readPixels(int x,
    ...
    //syslog(LOG_DEBUG, "[ColorBuffer] readPixels color buffer: %d\n", mHndl);
    if (fb->isOwnedByCaptureScreen(mHndl)) {
        //syslog(LOG_DEBUG, "[ColorBuffer][%s] w = %d, h = %d, buf w = %d, h = %d, color w = %d, h = %d\n", 
        //    __func__, width, height, fb->windowWidth(), fb->windowHeight(), m_width, m_height);

        if (!pGrabberHelper) {
            pGrabberHelper = std::make_shared<GrabberHelper>(m_display, width, height);
            syslog(LOG_DEBUG, "[ColorBuffer] create new GrabberHelper...\n");
        }

        if (pGrabberHelper) {
            if ((width != pGrabberHelper->getWidth() || height != pGrabberHelper->getHeight()) || 
                    (pGrabberHelper->IsScreenImageChanged())) {
                pGrabberHelper.reset();
                pGrabberHelper = std::make_shared<GrabberHelper>(m_display, width, height);
                syslog(LOG_DEBUG, "[ColorBuffer] create new GrabberHelper...\n");
            }
            pGrabberHelper->UpdateAndReadPixel(x, y, width, height, p_format, p_type, pixels);
        }
    }
    else {
...
bool ColorBuffer::bindToTexture() {
    ...
    if (fb->isOwnedByCaptureScreen(mHndl)) {
        RenderThreadInfo* tInfo = RenderThreadInfo::get();
        if (!tInfo->currContext.get()) {
            syslog(LOG_DEBUG, "[ColorBuffer][%s] Can't get context!", __func__, mHndl);
            return false;
        }
        touch();

        GLint prevArrayBuffer, prevElementBuffer, currTexBind, prevUnpackBuf, prevFrameBuffer;
        s_gles2.glGetIntegerv(GL_ARRAY_BUFFER_BINDING, &prevArrayBuffer);
        s_gles2.glGetIntegerv(GL_ELEMENT_ARRAY_BUFFER_BINDING, &prevElementBuffer);
        s_gles2.glGetIntegerv(GL_TEXTURE_BINDING_2D, &currTexBind);
        s_gles2.glGetIntegerv(GL_PIXEL_UNPACK_BUFFER_BINDING, &prevUnpackBuf);
        s_gles2.glGetIntegerv(GL_FRAMEBUFFER_BINDING, &prevFrameBuffer);
        GLint prevViewport[4] = {0,};
        s_gles2.glGetIntegerv(GL_VIEWPORT, prevViewport);

        if (!pGrabberHelper) {
            pGrabberHelper = std::make_shared<GrabberHelper>(m_display, m_width, m_height);
            syslog(LOG_DEBUG, "[ColorBuffer] create new GrabberHelper...\n");
        }

        if ((m_width != pGrabberHelper->getWidth() || m_height != pGrabberHelper->getHeight()) || 
            (pGrabberHelper->IsScreenImageChanged())) {
            pGrabberHelper.reset();
            pGrabberHelper = std::make_shared<GrabberHelper>(m_display, m_width, m_height);
            syslog(LOG_DEBUG, "[ColorBuffer] create new GrabberHelper...\n");
        ...
        bool updated = false;
        if (pGrabberHelper) {
            updated = pGrabberHelper->updataEglImage();
        }

        s_gles2.glBindBuffer(GL_ARRAY_BUFFER, prevArrayBuffer);
        s_gles2.glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, prevElementBuffer);
        s_gles2.glBindTexture(GL_TEXTURE_2D, currTexBind);
        s_gles2.glBindBuffer(GL_PIXEL_UNPACK_BUFFER, prevUnpackBuf);
        s_gles2.glBindFramebuffer(GL_FRAMEBUFFER, prevFrameBuffer);
        s_gles2.glViewport(prevViewport[0], prevViewport[1], prevViewport[2], prevViewport[3]);

        if (updated) {
            pGrabberHelper->bindEglImage();
        }
...
// 系统修改缩放比后需要注销或重启，因此该值只需程序启动时获取一次即可
static float ScreenDPI = getScreenDPI();

bool ColorBuffer::post(GLuint tex, float rotation, float dx, float dy, int windowWidth, int windowHeight) {
    // NOTE: Do not call m_helper->setupContext() here!
    waitSync();
    float widthRatio = 0;
    float heightRatio = 0;
    if (m_supportDynamicSize) {
        widthRatio = ((windowWidth / ScreenDPI) - 1) / (float)m_width;
        heightRatio = ((windowHeight / ScreenDPI) - 1) / (float)m_height;
    }
    //syslog(LOG_DEBUG, "######### windowWidth = %d, windowHeight = %d, ScreenDPI = %f, m_width = %d, m_height = %d",
    //    windowWidth, windowHeight, ScreenDPI, m_width, m_height);
    return m_helper->getTextureDraw()->draw(tex, rotation, dx, dy, widthRatio, heightRatio);
}

bool ColorBuffer::postWithOverlay(GLuint tex, float rotation, float dx, float dy, int windowWidth, int windowHeight) {
    // NOTE: Do not call m_helper->setupContext() here!
    waitSync();
    float widthRatio = 0;
    float heightRatio = 0;
    ...
    //syslog(LOG_DEBUG, "######### windowWidth = %d, windowHeight = %d, ScreenDPI = %f, m_width = %d, m_height = %d",
    //    windowWidth, windowHeight, ScreenDPI, m_width, m_height);
    return m_helper->getTextureDraw()->drawWithOverlay(tex, rotation, dx, dy, widthRatio, heightRatio);
}
