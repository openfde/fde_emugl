...
bool RendererImpl::StartRenderServer(char* addr, size_t addrLen) {
    ...
    if (!s_renderThread) {
        syslog(LOG_DEBUG,"renderThread get fail");
        return false;
    }
    //strncpy(s_renderAddr, addr, sizeof(s_renderAddr));

    s_renderThread->start();
    m_lock = s_renderThread->getmlock();
    return true;
}    
...
bool RendererImpl::showOpenGLSubwindow(int wx,
                                       int wy,
                                       int ww,
                                       int wh,
                                       int fbw,
                                       ...
                                       bool hideWindow) {
    assert(mRenderWindow);
    return mRenderWindow->setupSubWindow( wx, wy, ww, wh, fbw, fbh, dpr,
                                         zRot, deleteExisting, hideWindow);
}

bool RendererImpl::updateWindowAttri(uint32_t display_id,
    ...
    assert(mRenderWindow);
    m_lock->lock();
    bool result = mRenderWindow->updateWindowAttri(display_id,
                                                   win_id,
                                                   p_colorbuffer,
                                                   width,
                                                   height,
...
bool RendererImpl::deleteWindowAttri(uint32_t display_id)
{
    assert(mRenderWindow);
    m_lock->lock();
    bool result = mRenderWindow->deleteWindowAttri(display_id);
    m_lock->unlock();
    return result;
...
void RendererImpl::setSupportDynamicSize(uint32_t display_id, unsigned int p_colorbuffer,
                                   int32_t width, int32_t height,bool support) {
    assert(mRenderWindow);
    m_lock->lock();
    mRenderWindow->setSupportDynamicSize(display_id, p_colorbuffer, width, height, support);
    m_lock->unlock();
}
...
static struct AndroidVirtioGpuOps sVirtioGpuOps = {
        ...
        .post_color_buffer =
                [](uint32_t handle) { FrameBuffer::getFB()->post(0, handle, 540, 960, 0); },
