...
enum Command {
    ...
    CMD_FINALIZE,
    CMD_UPDATE_WINDOW_ATTRI,
    CMD_DELETE_WINDOW_ATTRI,
    CMD_SET_SUPPORT_DYNAMIC_SIZE,
};
    ...
    bool process() const {
        ...
        switch (msg.cmd) {
            ...
            case CMD_RESET_GUEST_POSTED_A_FRAME:
                ...
                break;
            case CMD_UPDATE_WINDOW_ATTRI:
            {    
                GL_LOG("CMD_UPDATE_WINDOW_ATTRI");
                D("CMD_UPDATE_WINDOW_ATTRI\n");
                fb = FrameBuffer::getFB();
                if (fb) {
                    fb->UpdateWindowAttri(
                        msg.post_data.display_id,
                        msg.post_data.win_id,
                        msg.post_data.p_colorbuffer,
                        msg.post_data.width,
                        msg.post_data.height,
                        msg.post_data.orientation,msg.post_data.needpost);
                    result = true;
                } else {
                    GL_LOG("CMD_UPDATE_WINDOW_ATTRI: no FrameBuffer");
                }
                break;
            }
            case CMD_DELETE_WINDOW_ATTRI:
            {    
                GL_LOG("CMD_DELETE_WINDOW_ATTRI");
                D("CMD_DELETE_WINDOW_ATTRI\n");
                fb = FrameBuffer::getFB();
                if (fb) {
                    result = fb->DeleteWindowAttri(msg.delete_display_id);                    
                } else {
                    GL_LOG("CMD_DELETE_WINDOW_ATTRI: no FrameBuffer");
                }
                break;
            }
            case CMD_SET_SUPPORT_DYNAMIC_SIZE:
            {
                GL_LOG("CMD_SET_SUPPORT_DYNAMIC_SIZE");
                D("CMD_SET_SUPPORT_DYNAMIC_SIZE\n");
                fb = FrameBuffer::getFB();
                if (fb) {
                    fb->supportDynamicSize(
                        msg.dds_data.display_id, 
                        msg.dds_data.p_colorbuffer, 
                        msg.dds_data.width, 
                        msg.dds_data.height, 
                        msg.dds_data.support_dynamic_size);
                    result = true;                 
                } else {
                    GL_LOG("CMD_SET_SUPPORT_DYNAMIC_SIZE: no FrameBuffer");
                }
                break;
            }
...
bool RenderWindow::setupSubWindow(int wx,
                                  int wy,
                                  int ww,
                                  int wh,
                                  int fbw,
...
bool RenderWindow::updateWindowAttri(uint32_t display_id,
                                  unsigned long win_id,
                                  unsigned int p_colorbuffer,
                                  int32_t width,
                                  int32_t height,
    ...
    RenderWindowMessage msg = {};
    msg.cmd = CMD_UPDATE_WINDOW_ATTRI;
    msg.post_data.display_id = display_id;
    msg.post_data.win_id = win_id;
    msg.post_data.p_colorbuffer = p_colorbuffer;
    msg.post_data.width = width;
    msg.post_data.height = height;
    msg.post_data.orientation = orientation;
    msg.post_data.needpost = needpost;
...
bool RenderWindow::deleteWindowAttri(uint32_t display_id) {
        
    RenderWindowMessage msg = {};
    msg.cmd = CMD_DELETE_WINDOW_ATTRI;
    msg.delete_display_id = display_id;    
...
bool RenderWindow::setSupportDynamicSize(uint32_t display_id, unsigned int p_colorbuffer,
                                   int32_t width, int32_t height, bool support) {
        
    RenderWindowMessage msg = {};
    msg.cmd = CMD_SET_SUPPORT_DYNAMIC_SIZE;
    msg.dds_data.display_id = display_id;
    msg.dds_data.p_colorbuffer = p_colorbuffer;
    msg.dds_data.width = width;
    msg.dds_data.height = height;
    msg.dds_data.support_dynamic_size = support;    
