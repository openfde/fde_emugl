...
typedef std::unordered_map<pthread_t, ColorBufferSet> PthreadOwnedColorBuffers;
...
//************************* add by hcl for save multi window info *************** start **************
struct RendererWindowAttri {  
  EGLNativeWindowType parent_window = 0;
  EGLNativeWindowType native_window = 0;
  EGLSurface surface = EGL_NO_SURFACE;
  EGLDisplay RW_Display;
  int width;
  int height;
  int rotation;
  HandleType lastPostedColorBuffer;
  bool disableShow = false;
  };

typedef std::map<int, RendererWindowAttri**> WindowsMap;

//************************* add by hcl for save multi window info *************** end **************
...
public:
    ...
    int GetRotation(int32_t orientation);
    void setDisplayShowStatus(int display_id, int32_t transfer_flags);
    void freeRenderWindow (RendererWindowAttri** window_attri_ptr);
    bool DeleteWindowAttri(int display_id);
    RendererWindowAttri* UpdateWindowAttri(int display_id, EGLNativeWindowType win_id, unsigned int p_colorbuffer, int32_t width, int32_t height, int32_t orientation, bool needpost);
    ...
    void closePthreadAloneColorBuffer(pthread_t tid);
    
    void closeAllColorBuffer(bool needLock);
    ...
    bool post(int display_id, HandleType p_colorbuffer, int32_t width, int32_t height, int32_t orientation, bool needLockAndBind = true);
    ...
    void addCaptureScreenColorBuffer(HandleType p_colorbuffer) { m_captureScreenColorBuffers.emplace(p_colorbuffer);}
    void removeCaptureScreenColorBuffer(HandleType p_colorbuffer) { m_captureScreenColorBuffers.erase(p_colorbuffer);}
    bool isOwnedByCaptureScreen(HandleType p_colorbuffer) { return m_captureScreenColorBuffers.count(p_colorbuffer);}
    void supportDynamicSize(uint32_t display_id, unsigned int p_colorbuffer, int32_t width, int32_t height, bool support);
    
private:
    FrameBuffer(int p_width, int p_height, bool useSubWindow);
    HandleType genHandle_locked();

    bool bindSubwin_locked(EGLSurface surface);
    ...
    void closePthreadColorBufferLocked(HandleType p_colorbuffer, bool forced = false);
    ...
    bool postImpl(int display_id, HandleType p_colorbuffer, int32_t width, int32_t height, int32_t orientation, bool needLockAndBind, bool repaint= false);
...
private:
    ...
    std::set<HandleType> m_captureScreenColorBuffers;
    ...
    PthreadOwnedColorBuffers m_pthreadOwnedColorBuffers;
