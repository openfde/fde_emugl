...
static std::string filter_extensions(const std::string& extensions, const std::vector<std::string>& whitelist) {
    ...
    auto extension_list = string_split(extensions, ' ');
...
static EGLint rcQueryEGLString_filter(EGLenum name, void* buffer, EGLint bufferSize) {
    FrameBuffer *fb = FrameBuffer::getFB();
    if (!fb) {
        return 0;
    }
    //std::string result = s_egl.eglQueryString(renderer->getDisplay(), name);
    std::string result = s_egl.eglQueryString(fb->getDisplay(), name);
    if (result.empty())
        return 0;
...
bool isGLESDynamicVersionEnabled() {
...
static EGLint rcGetGLString_filter(EGLenum name, void* buffer, EGLint bufferSize) {
    ...
    if (name == GL_VERSION) {        
        ...
        if (!emugl_feature_is_enabled(android::featurecontrol::Egl2egl)) {
...
static void rcFBPost(uint32_t colorBuffer)
{
    ...
    fb->post(0, colorBuffer,540, 960, 0 , true);
}
...
static uint32_t rcCreateBuffer(uint32_t size) {
    return rcCreateBuffer2(size, 1);//VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT
}
...
static int rcSetColorBufferVulkanMode(uint32_t colorBuffer, uint32_t mode) {
    return rcSetColorBufferVulkanMode2(colorBuffer, mode, 1);//VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT
}
...
static std::vector<VirtualDisplay_t> virtual_display_ts;

static void rcPostVirtualDisplay (const char* name, uint32_t id, uint32_t bo_name, int32_t width, int32_t height, int32_t stride, int32_t bpp, int32_t orientation) {
    VirtualDisplay_t display{
        id,
        bo_name,
        orientation,
    };
...
static void rcPostAllVirtualDisplaysDone() {
    FrameBuffer *fb = FrameBuffer::getFB();
    if (!fb) {
        return;
    }

    for (auto display : virtual_display_ts) {
...
static void rcPostCaptureScreenColorBuffer(uint32_t id) {
    FrameBuffer *fb = FrameBuffer::getFB();
    if (fb) {
        fb->addCaptureScreenColorBuffer(id);
    }
}
...
void initRenderControlContext(renderControl_decoder_context_t *dec) {
    ...
    dec->rcQueryEGLString = rcQueryEGLString_filter;
    dec->rcGetGLString = rcGetGLString_filter;
    ...
    dec->rcPostCaptureScreenColorBuffer = rcPostCaptureScreenColorBuffer;
    //****************************** add by hcl for postVirtualDisplay end
}
