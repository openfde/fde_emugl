...
FrameBuffer::postWorkerFunc(const Post& post) {
    RendererWindowAttri *cw_attri;
    if (post.cmd == PostCmd::Post || post.cmd == PostCmd::Viewport || post.cmd == PostCmd::Clear) {
        cw_attri = *(post.window_attri);
        if (cw_attri == NULL)
        {
...
bool FrameBuffer::setupSubWindow(int wx,
    ...
    if (root)
    {
        ...
        m_eglSurface = s_egl.eglCreateWindowSurface(
                    m_eglDisplay, m_eglConfig, root, NULL);
    ...
    if (cb.get() != NULL) {
        ...
        if (m_refCountPipeEnabled) {
            ...
            if (apiLevel > 0 && apiLevel < 26) {
                ...
                // set 'refcount' init value to '1' to avoid some colorbuffer 
                // released abnormally by color buffer memory management
                m_colorbuffers[handle] = {std::move(cb), 1, false, 0};
                m_pthreadOwnedColorBuffers[tid].insert(handle);
            }
        }
        ...
        if (winIt != m_windows.end()) {
            if (const HandleType oldColorBufferHandle = winIt->second.second) {
                if (m_refCountPipeEnabled) {
                    ...
                    // decrease all colorbuffer's 'refcount' owned by windowsurface
                    std::set<HandleType> colorbuffers = winIt->second.first->getOwnedColorBuffers();
...
std::vector<HandleType> FrameBuffer::DestroyWindowSurfaceLocked(HandleType p_surface) {
    ...
    if (w != m_windows.end()) {
        ...
        if (m_refCountPipeEnabled) {
            ...
            // decrease all colorbuffer's 'refcount' owned by windowsurface
            std::set<HandleType> colorbuffers = w->second.first->getOwnedColorBuffers();
            for (auto &cb : colorbuffers) {
                closeColorBufferLocked(cb);
            }
        }
...
void FrameBuffer::closeAllColorBuffer(bool neeLock) {
    ...
    std::vector<HandleType> allColorBuffers;
    for (auto it = m_colorbuffers.begin(); it != m_colorbuffers.end();)
    {        
        allColorBuffers.push_back((it++)->first);
    }
    for (auto iter = allColorBuffers.begin(); iter != allColorBuffers.end(); iter++)
    {
        closeColorBufferLocked(*iter);
    }
}
...
bool FrameBuffer::setWindowSurfaceColorBuffer(HandleType p_surface,
    ...
    // save windowsurface owned colorbuffer
    (*w).second.first->ownColorBuffer(p_colorbuffer);
...
typedef std::map<uint32_t, long> DeletedWindowsMap;
DeletedWindowsMap deletedWindowsMap;
...
void FrameBuffer::setDisplayShowStatus(int display_id, int32_t transfer_flags) {
    ...
    //syslog(LOG_DEBUG,"FrameBuffer setDisplayShowStatus AutoLock mutex(m_lock)...");
...
static int cmdGetResult(const char *cmd, char *result, int maxlen) {
    if (!result) {
        return -1;
    }
    ...
    FILE *pp = popen(cmd, "r");
    if (!pp) {
        fprintf(stderr,"error, cannot popen cmd: %s\n", cmd);
        return -1;
    }
    ...
    while (fgets(tmp, sizeof(tmp), pp) != NULL) {
        ...
        i += strlen(tmp);
        if (i >= maxlen) {
            fprintf(stderr,"get enough results, return\n");
            break;
        }
        memset(tmp, 0, sizeof(tmp));        
    }
...
static void getRuningWindowId(char * result) {
    if (!result) {
        return;
    }    
    cmdGetResult("ps -U $USER -u $USER -o pid,comm,cmd | grep kylin-kmre-window | grep -v grep | awk '{print $5}'", result, 1024);
    //syslog(LOG_DEBUG,"get_runing_window_id result = %s, get_lines = %d", result, get_lines);
    //fprintf(stderr,"get_runing_window_id result = %s, get_lines = %d\n", result, get_lines);
    return;
}

void FrameBuffer::freeRenderWindow (RendererWindowAttri** window_attri_ptr) {
    RendererWindowAttri* window_attri = NULL;
    if (window_attri_ptr) {            
        window_attri = *(window_attri_ptr);
...
bool FrameBuffer::DeleteWindowAttri(int display_id) {
    ...
    if (win != subWindowsMap.end()) {        
        RendererWindowAttri** window_attri_ptr = win->second;
        freeRenderWindow(window_attri_ptr);
        subWindowsMap.erase(win);

        long delete_time = System::get()->getUnixTime();
        auto de_win = deletedWindowsMap.find(display_id);
        if (de_win == deletedWindowsMap.end()) {            
            deletedWindowsMap.insert({display_id, delete_time});
        } else {
            deletedWindowsMap.erase(de_win);            
            deletedWindowsMap.insert({display_id, delete_time});
        }
        
        // char* ps_window_result = (char*)calloc(1024, sizeof(char));
        // if (ps_window_result) {
        //     getRuningWindowId(ps_window_result);            
        //     int tmp_id;
        //     for (auto iter = subWindowsMap.begin(); iter != subWindowsMap.end();)
        //     {
        //         tmp_id = iter->first;
        //         char display_id_str[128] = {0};
        //         snprintf(display_id_str, sizeof(display_id_str) + 1, "%d", tmp_id);
                
        //         if (!strstr(ps_window_result, display_id_str)) {//某个display_id，如果不存在相关联的window程序，需要从subWindowsMap直接删除
        //             syslog(LOG_DEBUG,"ps_window_result find window process failed! display_id = %d", tmp_id);
        //             fprintf(stderr,"ps_window_result find window process failed! display_id = %d\n", tmp_id);
        //             window_attri_ptr = iter->second;
        //             freeRenderWindow(window_attri_ptr);
        //             subWindowsMap.erase(iter++);
        //             delete_time = System::get()->getUnixTime();
        //             auto de_win = deletedWindowsMap.find(tmp_id);
        //             if (de_win == deletedWindowsMap.end()) {            
        //                 deletedWindowsMap.insert({tmp_id, delete_time});
        //             } else {
        //                 deletedWindowsMap.erase(de_win);            
        //                 deletedWindowsMap.insert({tmp_id, delete_time});
        //             }
        ...
        // }
        // if (ps_window_result) {
        //     free(ps_window_result);
        //     ps_window_result = NULL;
        // }
        ...
        if (!m_useSubWindow) {
            if (main_window_attri == NULL) {
                ...
                main_window_attri->native_window = root;
...
RendererWindowAttri* FrameBuffer::UpdateWindowAttri(int display_id, EGLNativeWindowType win_id, unsigned int p_colorbuffer, int32_t width, int32_t height, int32_t orientation, bool needpost) {       
    ...
    bool needCreateWindowAttri = true;
    if (win != subWindowsMap.end()) {
        ...
        if (window_attri) {
            if (window_attri->parent_window != win_id) {
                fprintf(stderr, "display_id = %d, FrameBuffer UpdateWindowAttri change win_id!\n",display_id);
                freeRenderWindow(window_attri_ptr);
                subWindowsMap.erase(win);            
            } else {
                if (m_useSubWindow) {
                ...
                needCreateWindowAttri = false;
            }
        }
        
    }

    if (needCreateWindowAttri) {          
        if (win_id) {
            //syslog(LOG_DEBUG, "########### m_useSubWindow = %d", m_useSubWindow);
            if (m_useSubWindow) {
            ...
            if (new_eglSurface == EGL_NO_SURFACE) {
                // NOTE: This can typically happen with software-only renderers
                // like OSMesa.
                //syslog(LOG_DEBUG,"display_id = %d, FrameBuffer UpdateWindowAttri eglCreateWindowSurface error!0x%x", display_id, s_egl.eglGetError());
                fprintf(stderr, "display_id = %d, FrameBuffer UpdateWindowAttri eglCreateWindowSurface error!0x%x\n", display_id, s_egl.eglGetError());
                return NULL;
            }
        }
        ...
        if (native_window) {
            ...
            window_attri->parent_window = win_id;
        ...
        postImpl(display_id, p_colorbuffer, width, height, orientation, false);
    }    
...
bool FrameBuffer::postImpl(int display_id, HandleType p_colorbuffer, int32_t width, int32_t height, int32_t orientation, bool needLockAndBind, bool repaint) {
    ...
    auto de_win = deletedWindowsMap.find(display_id);
    if (de_win != deletedWindowsMap.end()) {
        long delete_time = de_win->second;
        if (System::get()->getUnixTime() - delete_time < 5) {
            //fprintf(stderr,"FrameBuffer postImpl, app maybe nearly delete, post do nothing!\n");
            return false;
        }
    }
    ...
    if (m_window_attri->surface == EGL_NO_SURFACE) {
        ...
        if (new_eglSurface == EGL_NO_SURFACE) {
                // NOTE: This can typically happen with software-only renderers
                // like OSMesa.
                //syslog(LOG_DEBUG,"display_id = %d, FrameBuffer postImpl eglCreateWindowSurface error!0x%x", display_id, s_egl.eglGetError());
                fprintf(stderr, "display_id = %d, FrameBuffer postImpl eglCreateWindowSurface error!0x%x\n", display_id, s_egl.eglGetError());
                return false;
        }
...
bool FrameBuffer::compose(uint32_t bufferSize, void* buffer) {
    ...
    case 1: {
        ...
        post(0, p->targetHandle, 720, 1280, 0 ,false);
        return true;
    }
...
void FrameBuffer::supportDynamicSize(uint32_t display_id, unsigned int p_colorbuffer, int32_t width, int32_t height, bool support)
{
    (void)display_id;
    (void)width;
    (void)height;
    ...
    ColorBufferMap::iterator c(m_colorbuffers.find(p_colorbuffer));
    if (c != m_colorbuffers.end()) {
        c->second.cb->setSupportDynamicSize(support);
    }
}