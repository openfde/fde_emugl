...
public:
    ...
    // coordinate space.
    bool post(GLuint tex, float rotation, float dx, float dy, int windowWidth, int windowHeight);
    // Post this ColorBuffer to the host native sub-window and apply
    // the device screen overlay (if there is one).
    // |rotation| is the rotation angle in degrees, clockwise in the GL
    // coordinate space.
    bool postWithOverlay(GLuint tex, float rotation, float dx, float dy, int windowWidth, int windowHeight);
    ...
    void setSupportDynamicSize(bool support){m_supportDynamicSize = support;}
    bool isDynamicSizeSupported(){return m_supportDynamicSize;}
...
private:
    ...
    uint64_t m_pre_record_time;
    uint32_t m_record_counter;
    ...
    bool m_supportDynamicSize = false;
};
